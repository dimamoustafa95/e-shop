...
class ReviewController extends Controller
{
    public function add($product_slug)
    {
        ...
        $product = Product::query()->where('slug',$product_slug)->where('status','0')->first();
        if($product) {
    ...
    public function create(Request $request)
    {
        ...
        $product = Product::query()->where('id',$product_id)->where('status','0')->first();
        if($product){
    ...
    public function update(Request $request){
        ...
        if($user_review !=''){
            ...
            $review = Review::query()->where('id',$review_id)->where('user_id',Auth::id())->first();
            if($review){
